...
create_lorenz_rank <- function(df_proportion) {
  ...
  a <-ggplot(data = df_prop, aes(x= rank, y= proportion, color = year)) + 
    ...
    xlab('Rank') +
    ylab('Total Movement') +
    coord_cartesian(ylim = c(0, .5), xlim = c(0, 400)) + #CHANGE THIS SO THAT IT JUST DOES DF WITH MOST FISH
    theme_bw() +
  ...
  b <- ggplot(df_lorenz, aes(x=p, y=L, color = year_vector)) + geom_line() +
    scale_color_manual(values = cc) +
...
new_det <- read_csv("Z:/Shared/Projects/Evenness/CSVs/All Green Sturgeon Detections.csv", 
