...
```{r}
get_rank_plots_gini <- function(df_prop, pdf_name) {
  pdf(paste(pdf_name, ".pdf", sep = "" ),width = 11, height = 8)
  graph_list <- list()
  df_lm_equations <- NULL
  year_list <- determine_years_rank_plots(df_prop, df_all_years)
...
```{r}
get_lorenz_with_gini <- function(df_proportion, pdf_title) { #Need to work on this so that it's more autonomous and picks years selectively. Could be as simple as adding a unique function for all years since it should already be sorted by year.
  pdf(paste(pdf_title, '.pdf', sep = ''))
  all_years <- c(2006:2017)
  glist <- NULL
  for (y in all_years) {
    print(y)
    ind_year <- subset(df_proportion, year == y)
