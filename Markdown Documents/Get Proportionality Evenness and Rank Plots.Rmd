...
```{r}
df_all_years <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/water_year_data.csv", 
                         col_types = cols(end_date = col_character(), 
                                          start_date = col_character()))

new_det <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/GREEN STURGEON DETECTIONS CLEAN.csv", 
                    col_types = cols(DetectDate = col_datetime(format = "%Y-%m-%d %H:%M:%S")))


all_det <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/IsThisACSV.csv", 
                    col_types = cols(DetectDate = col_datetime(format = "%Y-%m-%d %H:%M:%S")))
all_det <- select(all_det, DetectDate, Detection_GPS, TagID, Location, Species, Release_Location, Release_Date, Release_GPS, Release_rkm = Release_RKm, Detect_rkm = Detection_RKm, Length, Length_Type, Tag_Type)
all_det <- subset(all_det, Species == 'White_Sturgeon' | Species == 'Striped_bass' | Species == 'Sacramento_pikeminnow' | Species == 'Steelhead')


gsturg_motility <- get_species_motility(new_det, 'Green_Sturgeon', 1300, df_all_years)
wsturg_motility <- get_species_motility(all_det, 'White_Sturgeon', 600, df_all_years)
striped_motility <- get_species_motility(all_det, 'Striped_bass', 580, df_all_years)
pike_motility <- get_species_motility(all_det, 'Sacramento_pikeminnow', 310, df_all_years)
steelhead_motility <- get_species_motility(all_det, 'Steelhead',350, df_all_years)


lm_equations_gs <- gsturg_motility %>% get_proportionality() %>% get_rank_plots_gini('Green Sturgeon All Years with Gini Coefficient')
lm_equations_ws <- wsturg_motility %>% get_proportionality() %>% get_rank_plots_gini('White Sturgeon All Years with Gini Coefficient')
lm_equations_striped <- striped_motility %>% get_proportionality() %>% get_rank_plots_gini('Striped Bass All Years with Gini Coefficient')
lm_equations_pike <- pike_motility %>% get_proportionality() %>% get_rank_plots_gini('Pikeminnow All Years with Gini Coefficient')
lm_equations_sth <- steelhead_motility %>% get_proportionality() %>% get_rank_plots_gini('Steelhead All Years with Gini Coefficient') # Not Working

gsturg_motility %>% get_proportionality() %>% get_lorenz_with_gini('Green Sturgeon Lorenz', 'Green Sturgeon Lorenz Plots')
wsturg_motility %>% get_proportionality() %>% get_lorenz_with_gini('White Sturgeon Lorenz', 'White Sturgeon Lorenz Plots')
striped_motility %>% get_proportionality() %>% get_lorenz_with_gini('Striped Bass Lorenz', 'Striped Bass Lorenz Plots')
pike_motility %>% get_proportionality() %>% get_lorenz_with_gini('Pikeminnow Lorenz', 'Pikeminnow Lorenz Plots')
steelhead_motility %>% get_proportionality() %>% get_lorenz_with_gini('Steelhead Lorenz', 'Steelhead Lorenz Plots')
```
...
gsturg_motility <- get_species_motility(new_det, 'Green_Sturgeon', 1300, df_all_years)
#new_mot <- gsturg_motility[!(gsturg_motility$total_rkm <= 2 & substr(gsturg_motility$last_location, 1, 2) == 'GG'), ] #removes fish only detected at GG bridge

lm_equations_gs <- new_mot %>% get_proportionality() %>% get_rank_plots_proportions_new_pielous('Test Binning') 
#OR
lm_equations_gs <- new_mot %>% get_proportionality() %>% get_rank_plots_proportions('PDF Name')
#OR
lm_equations_gs <- new_mot %>% get_proportionality() %>% get_rank_plots_gini('GS All Years with Gini Coefficient')
```
...
#steelhead_motility <- get_species_motility(new_det, 'Steelhead',350)
