...
all_years_evenness_species <- function(df, species_name) { #df should be a motility data.frame that has gone through the smush_motility function.
  ...
  ggplot(data = df_prop, aes(x=rank, y= proportion)) + 
    ...
    coord_cartesian(ylim = c(0, .4), xlim <- c(1,nrow(df_prop))) + 
    annotate('text', x=p_location, y=.35, label = paste("Pielou's Index:", pielou_index, sep = ' '), size = 3.5) + #change text on plot
    theme_bw() +
...
all_det <- read_csv("Z:/Shared/Projects/Evenness/CSVs/All Core Array Detections.csv", 
                    col_types = cols(DetectDate = col_datetime(format = "%Y-%m-%d %H:%M:%S")))

gs_det <- read_csv("Z:/Shared/Projects/Evenness/CSVs/All Green Sturgeon Detections.csv", 
                   col_types = cols(DetectDate = col_datetime(format = "%Y-%m-%d %H:%M:%S")))

df_years <- read_csv("Z:/Shared/Projects/Evenness/CSVs/water_year_data.csv", 
