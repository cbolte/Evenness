...
gs_det <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/GREEN STURGEON DETECTIONS CLEAN.csv", 
                   col_types = cols(DetectDate = col_datetime(format = "%Y-%m-%d %H:%M:%S")))

df_years <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/water_year_data.csv", 
                         col_types = cols(end_date = col_character(), 
                                          start_date = col_character()))

det <- select(all_det, DetectDate, Detection_GPS, TagID, Location, Species, Release_Location, Release_Date, Release_GPS, Release_rkm = Release_RKm, Detect_rkm = Detection_RKm, Length, Length_Type, Tag_Type)
det <- subset(det, Species == 'White_Sturgeon' | Species == 'Striped_bass' | Species == 'Sacramento_pikeminnow' | Species == 'Steelhead')


#Get Motilities
gsturg_motility <- get_species_motility(gs_det, 'Green_Sturgeon', 1300)
...
#Assemble Plots of All Fish

a <- all_years_evenness_gini(all_steelhead, 'All Steelhead Evenness')
b <- all_years_evenness_gini(all_striped, 'All Striped Bass Evenness')
c <- all_years_evenness_gini(all_white, 'All White Sturgeon Evenness')
d <- all_years_evenness_gini(all_green, 'All Green Sturgeon Evenness')
e <- all_years_evenness_gini(all_pike, 'All Sacramento Pikeminnow Evenness')
...
#Make Lorenz Charts of Each Fish
...
lorenz_list <- list(f, g, h, i, j)
do.call('grid.arrange', c(lorenz_list, top = 'Interspecies Lorenz Charts with Gini Index'))
