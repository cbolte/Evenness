...
longterm_individual_evenness <- function(df, tagid) {
  ...
  gini_index <- signif(ineq(df_prop$proportion), 3)
  
  ggplot(data = df_prop, aes(x= index, y= proportion)) + 
    geom_point() + 
    geom_line() +
    ...
    annotate('text', x=nrow(df_prop)/3, y=1, label = paste('Gini Index:', gini_index, sep = ' '), size = 2.5) +
...
Runthrough Code:
```{r}
det <- read_csv("C:/Users/cbolte/Desktop/R Projects/Evenness/Excel and Csv/IsThisACSV.csv")
...
new_det <- select(det, DetectDate, Detection_GPS, TagID, Location, Species, Release_Location, Release_Date, Release_GPS, Release_rkm = Release_RKm, Detect_rkm = Detection_RKm, Length, Length_Type, Tag_Type)
...
get_longterm_plots(ws_longterm_prop, 'Longterm White Sturgeon Evenness')
longterm_lorenz_with_gini(ws_longterm_prop, 'Longterm White Sturgeon Lorenz')
...
longterm_lorenz_with_gini(ws_longterm_prop, 'Test Longterm Lorenz')
```
